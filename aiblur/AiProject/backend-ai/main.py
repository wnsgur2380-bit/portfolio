...
def process_video_for_privacy(video_path: str, post_id: int) -> dict:
    try:
        ...
        while cap.isOpened():
            ...
            # 1. 차량 탐지 (번호판 필터링용) - imgsz 조정하여 성능 유지
            ...
            # 2. 얼굴 인식 (매 프레임 실행 - 스킵 없음) - imgsz 조정
            face_results = face_model.track(frame, conf=0.20, imgsz=640, augment=False, persist=True, tracker="botsort.yaml", verbose=False)
            if face_results:
            ...
            # 3. 번호판 인식 (고급 로직) - imgsz 조정
            plate_results = plate_model.track(frame, conf=0.08, imgsz=960, augment=False, persist=True, tracker="botsort.yaml", verbose=False)
            current_frame_ids = []
